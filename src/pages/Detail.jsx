...
const Detail = function Detail(props) {
    ...
    useEffect(() => {
        (async () => {
            ...
            if (infos.base.info && !infos.store.list) {
                dispatch(queryStoreList());
            }
        })();
    }, [infos]);
    // 依赖于收藏列表和路径参数,计算出是否收藏
    const isStore = useMemo(() => {
        if (!infos.store.list) return false;
        return infos.store.list.some(item => {
            return +item.news.id === +params.id;
        });
    }, [infos.store.list, params]);
    ...
    const handleStore = async () => {
        ...
        if (isStore) {
            // 移除收藏
            let item = infos.store.list.find(item => {
                return +item.news.id === +params.id;
            });
            if (!item) return;
        ...
        try {
            ...
            dispatch(queryStoreList()); //同步最新的收藏列表到redux容器中
        } catch (_) { }
    };
